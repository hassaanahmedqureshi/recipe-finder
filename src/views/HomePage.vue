    ...
    <div v-if="recipes.length" class="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <a :href="recipe.url" target="_blank" class="block">
          ...
          <button  @click="getRecipes(true)"  class="w-full max-w-md p-2 bg-green-500 text-white rounded hover:bg-green-600 transition">
            Find Recipes</button>
...
export default {
  ...
  async getRecipes(isNewSearch = false) {
    ...
    // Clear previously searched recipes only for a new search
    if (isNewSearch) {
      this.recipes = []; 
      this.currentPage = 1; // Reset to the first page for the new search
    }
  ...
  handleScroll() {
    ...
    if (currentScroll >= scrollable - 100 && !this.loading && this.recipes.length < this.totalHits) {
      this.getRecipes(); // Call without clearing recipes
    }
  }
},
  mounted() {
